...
interface ExternalMatchRow {
  ...
  audit: {
    builtBy: "scripts/match_ucdp_candidate_events.ts";
...
const EXTERNAL_PATH = "data/external/ucdp_candidate/candidate_events_compact.jsonl";
const PROFILE_PATH = "data/external/ucdp_candidate/candidate_profile.json";
const OUTPUT_PATH = "data/external/matches/phase2_ucdp_candidate_matches.jsonl";
const REPORT_PATH = "data/external/matches/phase2_ucdp_candidate_match_report.json";
...
function printHelp() {
  console.log(`HopeIndexAI UCDP Candidate matcher
...
function dateDistanceDays(recordDay: number | null, external: CompactUcdpCandidateEvent): number | null {
...
function scoreCandidate(record: TrainingRecord, external: CompactUcdpCandidateEvent, targetCountryKey: string): ExternalMatchCandidate | null {
  ...
  return {
    sourceSystem: "ucdp_candidate",
    datasetVersion: external.dataset.version,
    evidenceTier: "external_curated",
...
async function loadExternalCandidates(countryKeys: Set<string>, minDay: number, maxDay: number): Promise<CompactUcdpCandidateEvent[]> {
  const candidates: CompactUcdpCandidateEvent[] = [];
  ...
  for await (const line of rl) {
    ...
    let row: CompactUcdpCandidateEvent;
    try {
      row = JSON.parse(line) as CompactUcdpCandidateEvent;
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      throw new Error(`Invalid compact UCDP Candidate JSONL line ${lineNumber}: ${message}`);
    }
...
async function main() {
  ...
  const [records, profile] = await Promise.all([
    ...
    readFile(PROFILE_PATH, "utf8").then((text) => JSON.parse(text) as CandidateProfile),
  ]);
  ...
  const candidates = profileMinDay === null || profileMaxDay === null
    ? []
    : await loadExternalCandidates(countryKeys, minRecordDay - DATE_WINDOW_DAYS, maxRecordDay + DATE_WINDOW_DAYS);
  ...
  for (const record of records) {
    ...
    if (
      ...
      outputRows.push(makeEmptyRow(
        record,
        "no_temporal_overlap",
        `UCDP Candidate date range ends at ${profile.dateRange?.max ?? "unknown"}; record date is ${record.event?.occurredOn ?? "unknown"}.`
      ));
      continue;
    }
    ...
    outputRows.push({
      ...
      audit: {
        ...
        notes: matches.length > 0
          ? "Candidate external evidence found. It supports curated current organized-violence evidence only; it does not create source-checked human importance truth."
          : "No UCDP Candidate row met the conservative date/country/location/actor/type score threshold.",
      },
    });
  }
  ...
  const report = {
    ...
    dateWindowDays: DATE_WINDOW_DAYS,
    minScore: MIN_SCORE,
    externalCandidatesLoaded: candidates.length,
    ...
    notes: [
      ...
      "UCDP Candidate covers current organized lethal violence, not all protests, diplomacy, disasters, or economic shocks.",
    ],
  };

  await writeFile(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`);

  console.log("HopeIndexAI UCDP Candidate matching");
  console.log(`Records checked: ${records.length}`);
  console.log(`External candidates loaded into matching window: ${candidates.length}`);
  console.log(`Records with UCDP Candidate matches: ${report.recordsWithCandidateMatches}`);
...
main().catch((err) => {
  console.error("HopeIndexAI UCDP Candidate matching failed:", err instanceof Error ? err.message : err);
  process.exit(1);
});
